...
namespace Exercises
{
    public partial class Exercises
    {
        ...
        {
            foreach(KeyValuePair<string, int> kpv in remoteWarehouse)
            {
                if (mainWarehouse.ContainsKey(kpv.Key))
                {
                    mainWarehouse[kpv.Key] += kpv.Value;
                }
                else
                {
                    mainWarehouse.Add(kpv.Key, kpv.Value);
                }
            }
