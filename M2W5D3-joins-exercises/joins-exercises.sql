...
JOIN film_actor fa ON fa.film_id = f.film_id
JOIN actor a ON fa.actor_id = a.actor_id
WHERE (a.first_name = 'RITA' AND a.last_name = 'REYNOLDS') OR (a.first_name = 'RITA' AND a.last_name = 'REYNOLDS');
...
-- (2 rows)
SELECT a.address, c.city, a.district, co.country FROM store s
JOIN address a ON a.address_id = s.address_id
JOIN city c ON c.city_id = a.address_id
JOIN country co ON co.country_id = c.country_id;
-- 12. A list of all stores by ID, the store’s street address, and the name of the store’s manager
-- (2 rows)
SELECT s.store_id, a.address, (st.first_name + ' ' + st.last_name) AS store_manager FROM store s
JOIN address a ON a.address_id = s.address_id
JOIN staff st ON st.staff_id = s.manager_staff_id;

-- 13. The first and last name of the top ten customers ranked by number of rentals 
-- (#1 should be “ELEANOR HUNT” with 46 rentals, #10 should have 39 rentals)
SELECT TOP 10 c.first_name, c.last_name, COUNT(r.customer_id) AS rentals FROM customer c
...
SELECT TOP 10 c.first_name, c.last_name, SUM(p.amount) AS amount FROM customer c
